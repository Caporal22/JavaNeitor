...
//            throw new RuntimeException(e);
        ...
        // Si no existe, cargamos algunos snakcs iniciales
        if (!existe){
    ...
    private List<Snack> obtenerSnacks(){
        ...
        try{ // Leemos cada una de las líneas
            List<String> lineas = Files.readAllLines(Paths.get(NOMBRE_ARCHIVO));
            for(String linea : lineas){
                String[] lineaSnack = linea.split(","); // PARSEO Acceso a cada uno de los elementos separados por las comas
                var idSnack = lineaSnack[0]; // No lo utilizamos
                var nombre = lineaSnack[1];
                var precio = Double.parseDouble(lineaSnack[2]);
                var snack = new Snack(nombre, precio);
                snacks.add(snack); // Agrega snack a la lista
            }
        }catch (Exception e){
